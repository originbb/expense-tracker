...
- **📸 영수증 자동 스캔 (OCR)**
  ...
  - 다중 이미지 일괄 스캔을 지원합니다.
  - *Tip: 선명하게 촬영되고 구겨짐이 없는 영수증일수록 한글/숫자 인식률이 크게 향상됩니다.*
- **📅 달력 기반 직관적 UI**
...
> **💡 Tip:** `.env` 파일에 `TURSO_DATABASE_URL` 정보가 설정되어 있지 않으면 기본적으로 로컬 파일 DB(`local.db`) 모드로 동작합니다. 클라우드 DB 세팅 없이도 로컬 환경에서 즉시 실행하고 테스트할 수 있습니다.
